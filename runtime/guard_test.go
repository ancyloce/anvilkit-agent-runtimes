...
func boundary(t *testing.T) *Boundary {
	t.Helper()
	value, err := NewBoundary(testControlPlane, []string{PathModelInvocations, PathRuntimeArtifacts})
	if err != nil {
...
func TestADestinationIsTheOriginPlusAReleasedPathAndNothingElse(t *testing.T) {
	guard := boundary(t)
	destination, err := guard.Resolve(PathModelInvocations)
	if err != nil {
		t.Fatalf("a released path had no destination: %v", err)
	}
	if destination != testControlPlane+PathModelInvocations {
		t.Fatalf("destination = %q", destination)
	}
	// Exact match, deliberately. A prefix or suffix rule lets a lookalike route
	// through, and a runtime that reaches a lookalike has selected an endpoint.
	for _, lookalike := range []string{
		PathModelInvocations + "/../../elsewhere",
		PathModelInvocations + "2",
		"/v1/internal/runtime/model-invocation",
		PathContractRuntimeInvocations,
		"",
	} {
		if _, err := guard.Resolve(lookalike); err == nil {
			t.Fatalf("a path this release does not carry was resolved: %q", lookalike)
...
func TestOnlyReleasedControlPlanePathsAreReachable(t *testing.T) {
	guard := boundary(t)
	for _, released := range []string{PathModelInvocations, PathRuntimeArtifacts} {
		if err := guard.AllowControlPlane(released); err != nil {
			t.Fatalf("a released path was refused: %q", released)
		}
	}
	for _, outside := range []string{"/v1/runs", PathModelInvocations + "/", "/v1/internal/runtime/artifacts/../runs", "", "/v1/"} {
...
func TestABoundaryCannotBeBuiltWithoutAGovernedOrigin(t *testing.T) {
	if _, err := NewBoundary("", nil); err == nil {
		t.Fatal("a boundary with no control-plane origin was built")
	}
	if _, err := NewBoundary("   ", nil); err == nil {
		t.Fatal("a boundary with a blank control-plane origin was built")
	}
	...
	if _, err := NewBoundary(testControlPlane, []string{"https://elsewhere.example/"}); err == nil {
